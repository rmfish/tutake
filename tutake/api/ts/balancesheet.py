...
from tutake.api.ts.balancesheet_ext import *
...
class TushareBalancesheet(Base):
    __tablename__ = "tushare_balancesheet"
...
class Balancesheet(BaseDao, TuShareBase, DataProcess):
    instance = None
    ...
    def __init__(self, config):
        self.engine = create_engine(config.get_data_sqlite_driver_url('tushare_balancesheet.db'),
                                    connect_args={'check_same_thread': False})
        session_factory = sessionmaker()
        session_factory.configure(bind=self.engine)
        TushareBalancesheet.__table__.create(bind=self.engine, checkfirst=True)
        ...
        BaseDao.__init__(self, self.engine, session_factory, TushareBalancesheet, 'tushare_balancesheet', query_fields,
                         entity_fields, config)
        DataProcess.__init__(self, "balancesheet", config)
        TuShareBase.__init__(self, "balancesheet", config, 2000)
    ...
    def balancesheet(self, fields='', **kwargs):
        """
        获取上市公司资产负债表
        | Arguments:
    ...
    def fetch_and_append(self, **kwargs):
        ...
        def fetch_save(offset_val=0):
            try:
                ...
                self.logger.debug("Invoke pro.balancesheet with args: {}".format(kwargs))
                res = self.tushare_query('balancesheet', fields=self.entity_fields, **kwargs)
                res.to_sql('tushare_balancesheet',
                           con=self.engine,
                           if_exists='append',
                           index=False,
                           index_label=['ts_code'])
...
setattr(Balancesheet, 'default_limit', default_limit_ext)
...
if __name__ == '__main__':
    ...
    print(pro.balancesheet())

    api = Balancesheet(config)
    api.process()    # 同步增量数据
    print(api.balancesheet())    # 数据查询接口
