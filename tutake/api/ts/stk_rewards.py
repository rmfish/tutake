...
from tutake.api.ts import stk_rewards_ext
...
class TushareStkRewards(TutakeTableBase):
...
class StkRewards(TushareDAO, TuShareBase, DataProcess):
    ...
    def __init__(self, config):
        ...
        self.writer = BatchWriter(self.engine, self.table_name, BaseDao.parquet_schema(TushareStkRewards),
                                  config.get_tutake_data_dir())

        query_fields = ['ts_code', 'end_date', 'limit', 'offset']
        self.tushare_fields = ["ts_code", "ann_date", "end_date", "name", "title", "reward", "hold_vol"]
    ...
    def fetch_and_append(self, **kwargs):
        ...
        def fetch_save(offset_val=0):
            try:
                ...
                return self.tushare_query('stk_rewards', fields=self.tushare_fields, **kwargs)
...
extends_attr(StkRewards, stk_rewards_ext)
