...
class IndexMonthly(TushareDAO, TuShareBase, DataProcess):
    ...
    def __init__(self, config):
        self.engine = create_engine(config.get_data_sqlite_driver_url('tushare_index_monthly.db'),
                                    connect_args={
        ...
        TushareDAO.__init__(self, self.engine, session_factory, TushareIndexMonthly, 'tushare_index_monthly.db',
                            'tushare_index_monthly', query_fields, entity_fields, config)
    ...
    def fetch_and_append(self, **kwargs):
        ...
        def fetch_save(offset_val=0):
            try:
                ...
                res = self.tushare_query('index_monthly', fields=self.entity_fields, **kwargs)
                res.to_sql('tushare_index_monthly',
                           con=self.engine,
                           if_exists='append',
                           index=False,
                           index_label=['ts_code'])
