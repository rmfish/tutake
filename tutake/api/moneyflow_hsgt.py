...
engine = create_engine(
    "%s/%s" % (config['database']['driver_url'], 'tushare_moneyflow_hsgt.db'))
...
class MoneyflowHsgt(BaseDao, TuShareBase):
    ...
    def __init__(self):
        BaseDao.__init__(self, engine, session_factory, TushareMoneyflowHsgt,
    ...
    def moneyflow_hsgt(self, **kwargs):
        ...
        query = session_factory().query(TushareMoneyflowHsgt).filter_by(
            **params)
    ...
    def fetch_and_append(self, process_type: ProcessType, **kwargs):
        ...
        def fetch_save(offset_val=0):
            ...
            logger.debug(
                "Invoke pro.moneyflow_hsgt with args: {}".format(kwargs))
            fields = [
