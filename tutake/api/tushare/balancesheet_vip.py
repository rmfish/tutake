...
engine = create_engine("%s/%s" % (tutake_config.get_data_sqlite_driver_url(), 'tushare_balancesheet_vip.db'))
session_factory = sessionmaker()
session_factory.configure(bind=engine)
Base = declarative_base()
logger = logging.getLogger('api.tushare.balancesheet_vip')
...
TushareBalancesheetVip.__table__.create(bind=engine, checkfirst=True)
...
class BalancesheetVip(BaseDao, TuShareBase, DataProcess):
    ...
    def __init__(self):
        ...
        BaseDao.__init__(self, engine, session_factory, TushareBalancesheetVip, 'tushare_balancesheet_vip',
                         query_fields, entity_fields)
        TuShareBase.__init__(self)
        DataProcess.__init__(self, "balancesheet_vip")
        self.dao = DAO()
    ...
    def fetch_and_append(self, **kwargs):
        ...
        def fetch_save(offset_val=0):
            ...
            logger.debug("Invoke pro.balancesheet_vip with args: {}".format(kwargs))
            res = self.tushare_api().balancesheet_vip(**kwargs, fields=self.entity_fields)
            res.to_sql('tushare_balancesheet_vip', con=engine, if_exists='append', index=False, index_label=['ts_code'])
            return res
