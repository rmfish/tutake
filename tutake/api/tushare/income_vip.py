...
数据接口-沪深股票-财务数据-利润表  https://tushare.pro/document/2?doc_id=33
...
engine = create_engine("%s/%s" % (tutake_config.get_data_sqlite_driver_url(), 'tushare_income_vip.db'))
session_factory = sessionmaker()
session_factory.configure(bind=engine)
Base = declarative_base()
logger = logging.getLogger('api.tushare.income_vip')
...
TushareIncomeVip.__table__.create(bind=engine, checkfirst=True)
...
class IncomeVip(BaseDao, TuShareBase, DataProcess):
    ...
    def fetch_and_append(self, process_type: ProcessType, **kwargs):
        ...
        def fetch_save(offset_val=0):
            ...
            logger.debug("Invoke pro.income_vip with args: {}".format(kwargs))
            res = self.tushare_api().income_vip(**kwargs, fields=self.entity_fields)
            res.to_sql('tushare_income_vip', con=engine, if_exists='append', index=False, index_label=['ts_code'])
            return res
