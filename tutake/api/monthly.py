...
engine = create_engine(
    "%s/%s" % (config['database']['driver_url'], 'tushare_monthly.db'))
...
class Monthly(BaseDao, TuShareBase):
    ...
    def __init__(self):
        BaseDao.__init__(self, engine, session_factory, TushareMonthly,
                         'tushare_monthly')
    ...
    def fetch_and_append(self, process_type: ProcessType, **kwargs):
        ...
        def fetch_save(offset_val=0):
            ...
            res = pro.monthly(**kwargs, fields=fields)
            res.to_sql('tushare_monthly',
                       con=engine,
                       if_exists='append',
                       index=False,
                       index_label=['ts_code'])
